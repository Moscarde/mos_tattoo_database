...
DATABASES = {
    'default': {
        ...
        'NAME': os.getenv('POSTGRES_DB', 'tattoo_sales_db'),
        'USER': os.getenv('POSTGRES_USER', 'tattoo_user'),
        'PASSWORD': os.getenv('POSTGRES_PASSWORD', 'tattoo_password_2026'),
...
class Unit(models.Model):
    ...
    class Meta:
        db_table = 'units'
        managed = False  # Não deixa Django gerenciar a tabela
...
### 4. Use as Views para Queries Otimizadas

```python
# Usando as views do banco
from django.db import connection

def get_sales_complete():
    with connection.cursor() as cursor:
        cursor.execute("SELECT * FROM v_sales_complete LIMIT 100")
        columns = [col[0] for col in cursor.description]
        return [dict(zip(columns, row)) for row in cursor.fetchall()]
```

### 5. Exemplo de View Django
...
def dashboard_metrics(request):
    ...
    # Vendas por unidade
    sales_by_unit = Sale.objects.filter(
        sold_at__gte=last_30_days,
        canceled=False
...
**Desenvolvido com ❤️ para análise de dados e BI**
